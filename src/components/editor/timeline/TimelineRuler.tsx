...
export const TimelineRuler: React.FC<TimelineRulerProps> = ({ pixelsPerSecond, scrollLeft, onSeek }) => {
  ...
  return (
    <div
      className="relative h-8 bg-[#171a1f] border-b border-[#2c2f34] cursor-pointer select-none overflow-hidden"
      ...
      {markers.map((time) => {
        ...
        return (
          ...
          >
            ...
            {isMajor && (
              <span className="absolute top-4 left-1 text-[10px] leading-none text-[#7f8894] group-hover:text-[#d0d6de]">
                {formatTime(time)}
              </span>
            )}
          </div>
